...
class AnalysisOfDeltaV:
    def __init__(self, stages: list, jettison_mass: list = None,
                 payload_mass: float = 0.0, h_target=0, name: str = 'Rocket'):
    ...
    def update_h_target(self, h_target):
        self.h_target = h_target
    ...
    def get_minimal_prop_qtd(self, deltaV_target: float = None, stage_position='last', efic: float = 0,
                             mpay: float = None, max_prop: float = 10000, step=1):
        if deltaV_target is None and self.h_target is not None:
            if self.h_target != 0:
                deltaV_target = self.Min_deltaV_LEO(h=self.h_target)
        if mpay is None:
            pass
        else:
            self.payload_mass = mpay
    ...
    def Min_deltaV_LEO(self, h=None):
        if h is None and self.h_target is not None:
            if self.h_target != 0:
                h = self.h_target
        R_e = 6378.1  # km
        u = 398600  # km3/s2
        ...
        print("Min DeltaV to LEO: orbit height: {:2f} km, Dv: {:.4f} km/s".format(h, DV_min))
