...
def Propellant_tank(r, Vp, P_int, rho, Sw, Fs):
    """ r           :   raio externo
    ...
    if Ve >= Vp:
        ...
        r_b = b(Vp, r)
        K_ratio = K(r, r_b)
        E = 2*K_ratio + (1/np.sqrt(K_ratio**2 - 1))*np.log((K_ratio+np.sqrt(K_ratio**2 - 1))/(K_ratio-np.sqrt(K_ratio**2-1)))
        tcilindro = tc(P_int, r, Sw)
        a = r - tcilindro
        R = K_ratio * a
        t_k = tk(P_int, K_ratio, a, Sw)
        t_cr = tcr(P_int, R, Sw)
        t_e = tee(t_k, t_cr)
        t_s = tse(P_int, a, K_ratio, Sw)
        t_c = tc(P_int, a, Sw)
        Vee = V_ee(a, b)
        Vse = V_se(a)
        ...
        Wsphe = Wse(a, t_s, rho)
        Wellip = Wee(a, t_e, E, rho, K_ratio)
